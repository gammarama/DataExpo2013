...
ggplot() + 
    geom_line(data = subset(south.dat.melt, Community == "Other"), 
              aes(x = year, y = as.numeric(value), group = QSB, colour=Community, shape=Community), 
              size = 1) + 
...
ggplot() + 
    geom_line(data = subset(myrtle.dat, Community == "Other"), 
              aes(x = disp_name, y = as.numeric(value), group = QSB, colour = Community.f, shape = Community.f), 
                  size = I(1)) + 
...
rust.dat$QSB <- factor(rust.dat$QSB)
levels(rust.dat$QSB) <- new.tbl$Community
ggplot() +
