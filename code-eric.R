...
clean.all.city <- ddply(clean.all, .(QSB), summarise, THRIVEIND = mean(THRIVEIND, na.rm = TRUE), AESTHETI = mean(AESTHETI, na.rm = TRUE), TOTALRESP = length(GENDER))
...
qplot(long, lat, data = states.data, group = group, geom = "polygon", fill = I("white")) +
    geom_path() +
    geom_point(data = clean.all.city, aes(lons, lats, colour = THRIVEIND, size = TOTALRESP), inherit.aes = FALSE) +
    scale_size(range = c(3, 8)) +
    geom_text(data = clean.all.city, aes(lons, lats, label = QSB), inherit.aes = FALSE, size = 3, vjust = -0.8) +
    scale_colour_gradient2(low = "red", high = "darkgreen", mid = "yellow", midpoint = median(clean.all.city$THRIVEIND)) +
    theme_bw() +
